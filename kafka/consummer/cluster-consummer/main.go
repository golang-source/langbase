...
import (
	...
	"log"
	"github.com/go-micro-out-parts/sarama-cluster"
)

var logger = log.New(os.Stderr, "[TEST]", log.LstdFlags)
var normalLog = log.New(os.Stdout, "[Normal]", log.LstdFlags)
var address  =[]string{"192.168.1.132:9092"}
func main()  {
	topic := []string{"mytopic1"}
...
Loop:
	...
	normalLog.Printf( "%s consume %d messages \n", groupId, successes)
}